...
export class ProfanityFilterService {
  ...
  public filterProfanity(
    ...
    const arr = currentState.getLettersWithPositions();

    const slices = this.slicers.map((slicer) =>
      slicer.createSlice(currentState, arr)
    );
    ...
    slices.forEach((slice) => {
      slice.forEach((subslice) => {
        const didAnythingForSubslice = this.replaceProfanityInSubSlice(
          currentState,
          subslice,
          userPlacedLetters
        );
  ...
  private replaceProfanityInSubSlice(
    ...
    const replace = () =>
      this.profanity.forEach((curse) => {
        const index = str.indexOf(curse);

        if (index === -1) {
          return;
        } else {
          const letters = subslice.slice(index, curse.length);

          letters.forEach((lwp) => {
            if (
              !userPlacedLetters.filter(
                (upl) => lwp.column === upl.column && lwp.row === upl.row
              ).length
            ) {
